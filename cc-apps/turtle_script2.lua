...
---@class TSFunction
---@field fn fun(motion: Motion, arg: string|number): boolean? function that performs some action, returns true for fail
---@field can_repeat boolean this symbol can be followed by a number to use as repeat count
---@field needs_name boolean? this symbol needs a string name following it

---@type { [string]: TSFunction }
local function_def = {
    f = { fn = Motion.forward, can_repeat = true },
    b = { fn = Motion.backward, can_repeat = true },
    u = { fn = Motion.up, can_repeat = true },
    d = { fn = Motion.down, can_repeat = true },
    l = { fn = Motion.left, can_repeat = true },
    r = { fn = Motion.right, can_repeat = true },
    enable = { fn = Motion.enable_dig, can_repeat = true },
    disable = { fn = Motion.disable_dig, can_repeat = true },
}

local function is_valid(cmd)
    if function_def[cmd] ~= nil then
        return true
    end
    local valid = { '[', ']', 'm', 'mark', 'g', 'goto' }
...
local function parse_cmd(cmd)
    ...
    local actions = {}
    local nest = {}
    local in_define_name = nil
    local in_define = nil

    -- Define or reset meta functions
    function_def['m'] = {
        fn = function(_, name)
            name = tostring(name)
            nav:mark_poi(name)
    ...
    function_def['mark'] = function_def['m']
    function_def['g'] = {
        fn = function(_, poi_name)
            poi_name = tostring(poi_name)
    ...
    function_def['goto'] = function_def['g']

    local i = 1
    while i <= #all_parts do
        local w = all_parts[i]
        local fn_def = function_def[w]
        if fn_def == nil then
        ...
        elseif fn_def.needs_name then
            local name = all_parts[i + 1]
            if name == nil then
                log:error('Invalid marker name', name)
                return
            end
            table.insert(actions, { fn_def.fn, name })
